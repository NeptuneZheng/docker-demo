...
public class MappingThread extends Thread {
	...
	private static Logger logger = LogManager.getLogger(MappingThread.class.getName());
							...
							if (fail == false) {
								...
								LocalFileUtilQA
										.writeToFile(
												...
												output, false);
								if(CSB2BEDIConfigQA.TP_ID.equals("COSU_UIF")||CSB2BEDIConfigQA.TP_ID.equals("COSU_MIGRATION")){
									String boxmlPath = recordModel.getActualFilePath()+".boxml";
									UIFFileParser parser = new UIFFileParser();
									String definitionXml = LocalFileUtilQA.readBigFile(DemoGroovy_Common_QA.definitionFilePath);
								//	EDIProcessResult actediResult = parser.edi2xml(output, definitionXml);
									String transformSettingStr = "{\"isFieldValueTrimLeadingSpace\"=\"false\",\"isFieldValueTrimRightSpace\"=\"false\"}";
									logger.info (transformSettingStr);
									EDIProcessResult actediResult = parser.edi2xml(output, definitionXml);
									String actS = actediResult.outputString.toString();
									if(FunctionHelperQA.isNotEmpty(actS)){
										FunctionHelperQA.prettyFormatXML(actS,boxmlPath);
									}
								//	String actStr = FunctionHelperQA.cleanXml(actS);
									//FunctionHelperQA.prettyFormatXML(actS,boxmlPath);
								//	LocalFileUtilQA.writeToFile(boxmlPath,actStr,false);
...
//			closeExcel(excel);		
		...
		public void compareErrorMsg(RecordModelQA recordModel){
			...
			if(CSB2BEDIConfigQA.TP_ID.equals("COSU_UIF")){
