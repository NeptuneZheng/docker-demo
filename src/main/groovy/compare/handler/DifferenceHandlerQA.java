...
public class DifferenceHandlerQA {
	...
	static Logger logger = LogManager.getLogger(DifferenceHandlerQA.class.getName());
...
//	private static Configuration config;
	...
	private static DifferenceHandlerQA differenceHandlerQA = null;
	private static File excel = null;
	private static XSSFWorkbook workbook = null;

	private DifferenceHandlerQA(){}

	public static DifferenceHandlerQA getInstance(){
		if(differenceHandlerQA==null){
			synchronized (DifferenceHandlerQA.class){
				if(differenceHandlerQA==null)
					differenceHandlerQA = new DifferenceHandlerQA();
			}
		}
		return differenceHandlerQA;
	}
...
//		}
    ...
    public void writeUIFDif(String e, String a, String s) throws Exception {
//		File excel = new File(excelPath);
//		if (excel.renameTo(excel)) {
        XSSFSheet XSSFSheet = workbook.getSheetAt(0);
        XSSFCellStyle cellstyle = workbook.createCellStyle();
        ...
        cellactcontent.setCellStyle(cellstyle);
        XSSFCell cellsimilitytcontent = row.createCell((int) 4);
        cellsimilitytcontent.setCellType(XSSFCell.CELL_TYPE_STRING);
        cellsimilitytcontent.setCellStyle(cellstyle);

        cellexpcontent.setCellValue(e);
        cellactcontent.setCellValue(a);
        cellsimilitytcontent.setCellValue(s);
//			FileOutputStream fOut = new FileOutputStream(excel);
//			workbook.write(fOut);
//			fOut.flush();
//			fOut.close();
...
//		}
    ...
    public void writeUIFDifInfo(String expectedFilePath, String actualFilePath,
                             String inputFilePath, String workingFolder, String simility) throws Exception {
...
//			XSSFWorkbook workbook = new XSSFWorkbook(new FileInputStream(excel));
        ...
        cellactinputfile.setCellStyle(style1);
        cellactinputfile.setCellType(XSSFCell.CELL_TYPE_STRING);
        XSSFCell cellsimility = row.createCell((int) 5);
        cellsimility.setCellStyle(style1);
        cellsimility.setCellType(XSSFCell.CELL_TYPE_STRING);
        ...
        cellactinputfile.setCellValue(inputFilePath.substring(inputFilePath
                .indexOf(workingFolder)
                + workingFolder.length() + 1));
        cellsimility.setCellValue(simility);
//			FileOutputStream fOut = new FileOutputStream(excel);
//			workbook.write(fOut);
//
//			fOut.flush();
